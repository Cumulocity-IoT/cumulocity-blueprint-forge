 ...
 // TODO: Phase II
  ...
  async configureApp() {
    ...
    for (let db of this.configStepData.dashboards) {
      ...
      const templateDetails = await this.loadTemplateDetails(db);
      ...
      await this.catalogService.createDashboard(this.app, dashboardConfiguration, db, templateDetails);
  ...
  async installMicroservice(microService: MicroserviceDetails): Promise<void> {
    ...
    this.templateCatalogSetupService.downloadBinary(microService.link)
  ...
  async installPlugin(plugin: PluginDetails): Promise<void> {
        ...
        if (widgetBinaryFound) {
            ...
            this.progressIndicatorService.setProgress(10);
            this.templateCatalogSetupService.downloadBinary(plugin.link)
                .subscribe(data => {
                    this.progressIndicatorService.setProgress(20);
                    const blob = new Blob([data], {
                        type: 'application/zip'
  ...
  async loadTemplateDetails(db: Dashboards): Promise<any> {
