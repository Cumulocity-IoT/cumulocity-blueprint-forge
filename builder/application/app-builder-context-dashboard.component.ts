                ...
                tabs.push(...await Promise.all(dashboardsInTabgroup.map(async (dashboard, i) => {
                    ...
                    if (isGroupTemplate) {
                        ...
                        if(dashboard.templateType && dashboard.templateType == 2) {
                            childAssets = (await this.inventoryService.listQuery({ __filter: { __and: [{ __or: [{__has: 'c8y_IsDevice' }, {__has: 'c8y_IsAsset'  } ]}, { type: dashboard.deviceId }] } }, this.LIST_FILTER)).data;
                        } else {
